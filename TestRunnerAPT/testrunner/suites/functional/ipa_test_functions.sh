...
TZ_SYSFS_FILES="integral_cutoff k_d k_i k_po k_pu slope policy power \
...
{
	index=0
	for i in $TEMP_ORIG ; do
		TRIP_FILE=$SYSFS_DIR/${TZ_PREFIX}${TZ_ID}/${TRIP_PREFIX}_${index}_temp
		if [[ ! -f $TRIP_FILE ]]; then
			echo "File: $TRIP_FILE not valid"
			return 1
		fi
