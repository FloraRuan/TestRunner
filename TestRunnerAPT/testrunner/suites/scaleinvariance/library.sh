...
start_tracing()
{
  ...
  echo "Enabling sched_wakeup,sched_wakeup_new,sched_cpu_load,sched_switch events trace for pid==${TASK_TRACE_PID}"
  echo 1 > ${TRACEDIR}/events/sched/sched_wakeup/enable
  echo 1 > ${TRACEDIR}/events/sched/sched_wakeup_new/enable
  echo 1 > ${TRACEDIR}/events/sched/sched_cpu_load/enable
  echo 1 > ${TRACEDIR}/events/sched/sched_switch/enable
  echo "pid==${TASK_TRACE_PID}" > ${TRACEDIR}/events/sched/sched_wakeup/filter
  echo "pid==${TASK_TRACE_PID}" > ${TRACEDIR}/events/sched/sched_wakeup_new/filter
  echo "pid==${TASK_TRACE_PID}" > ${TRACEDIR}/events/sched/sched_cpu_load/filter
  echo "pid==${TASK_TRACE_PID}" > ${TRACEDIR}/events/sched/sched_switch/filter
