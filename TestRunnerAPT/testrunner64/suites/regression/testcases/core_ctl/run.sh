    ...
    if [ -e /etc/init.qcom.post_boot.sh ];then
        ...
        if [ "$soc_id" -eq "294" ] || [ "$soc_id" -eq "295" ] || [ "$soc_id" -eq "313" ];then #msm8937
            ...
            if [ -e  $CPUHOTPLUG_DIR/cpu${BIGCPUS:0:1}/core_ctl ]; then
                ...
                cur_busy_up_thres=$(cat $CPUHOTPLUG_DIR/cpu${BIGCPUS:0:1}/core_ctl/busy_up_thres|$TOOLS/busybox awk {'print $1'})
                cur_busy_down_thres=$(cat $CPUHOTPLUG_DIR/cpu${BIGCPUS:0:1}/core_ctl/busy_down_thres|$TOOLS/busybox awk {'print $1'})
